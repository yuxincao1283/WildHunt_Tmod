...
namespace LimbusCompanyWildHunt.Content.Mounts
{
	...
	public class DullahanMount : ModMount
	{
		...
		public override void SetStaticDefaults() {
			...
			MountData.runSpeed = 3f; // The speed of the mount
			MountData.dashSpeed = 9f; // The speed the mount moves when in the state of dashing.
			MountData.flightTimeMax = 0; // The amount of time in frames a mount can be in the state of flying.

			// Misc
			MountData.fatigueMax = 0;
			MountData.buff = 6; // The ID number of the buff assigned to the mount.

			// Effects
			MountData.spawnDust = DustID.ShimmerSpark; // The ID of the dust spawned when mounted or dismounted.

			// Frame data and player offsets
			MountData.totalFrames = 9; // Amount of animation frames for the mount
		...
		public override void UpdateEffects(Player player) {
			...
			if (Math.Abs(player.velocity.X) > 4f) {
				...
				Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustID.MinecartSpark);
			}
		}
